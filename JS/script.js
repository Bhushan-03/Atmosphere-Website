...
function setDailyFC(data) {
    ...
    for (let i = 1; i <= 7; i++){
        ...
        `<div class="d${i}-fc relative grid largDesk:grid-cols-[1.5fr_1fr_3fr_1fr] w-full largeMob:max-tab:grid-cols-[1.5fr_0.5fr_1.5fr_0.8fr] smallMob:max-largeMob:grid-cols-[1.5fr_0.5fr_1.5fr_0.8fr] tab:max-lap:grid-cols-[1.5fr_0.5fr_1.5fr_0.8fr] desk:max-largDesk:grid-cols-[1.5fr_1fr_3fr_1fr] lap:max-desk:grid-cols-[1.5fr_0.5fr_1.2fr_0.8fr] items-center justify-center">
            <p class="tab:max-lap:w-full largeMob:max-tab:w-full smallMob:max-largeMob:w-full text-nowrap smallMob:max-mob:text-sm">${data.dailyData.dailyDay[i]}</p>
            <img class="w-10 h-10 mr-4 flex justify-self-center smallMob:max-mob:w-8 smallMob:max-mob:h-8 smallMob:max-mob:ml-auto smallMob:max-mob:mr-auto" src="Assets/icons/${getWeatherIcon(data.dailyData.dailyWCondition[i])}" alt="">
            <div class="desk:max-largDesk:grid desk:max-largDesk:grid-cols-[1fr_3fr_1fr] flex lap:max-desk:justify-around largeMob:max-tab:justify-around tab:max-lap:justify-around smallMob:max-largeMob:justify-around">
                <p class="p-1 min-temp w-[29.5%] min-w-[29.5%] smallMob:max-mob:text-sm">${data.dailyData.dailyMaxTemp[i]}°</p>
                <div class="w-full overflow-hidden rounded-2xl p-1 lap:max-desk:hidden largeMob:max-tab:hidden smallMob:max-largeMob:hidden tab:max-lap:hidden">
                ...
                <p class="p-1 max-temp w-[29.5%] smallMob:max-mob:text-sm">${data.dailyData.dailyMaxFeelsLike[i]}°</p>
            </div>
            <div class="relative grid grid-cols-2 w-full items-center">
                ...
                <p class="smallMob:max-mob:text-sm">${data.dailyData.dailyPrepProbability[i]}%</p>
            </div>
        </div>`;
    }
}
